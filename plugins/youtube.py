...
async def youtube_main(c: Client, m: Message):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton("نتائج اليوتيوب", callback_data="ntagyout " + str(m.from_user.id))] +
        [InlineKeyboardButton("تحميل من الرابط", callback_data="downyout " + str(m.from_user.id))],
        [InlineKeyboardButton("بحث اليوتيوب صوت", callback_data="searchyout " + str(m.from_user.id))] +
        [InlineKeyboardButton("بحث اليوتيوب فديو", callback_data="searchyoutvideo " + str(m.from_user.id))],
