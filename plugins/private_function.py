    ...
    if m.text == "المطورين":
        if sudo2(m):
            try:
                ...
                if lang is None:
                    await m.reply_text(f"◍ [Black](tg://user?id={super_sudoers[0]})\n" +
                                       f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n"
                                       "لا يوجد مطورين مرفوعين\n√",
                                       parse_mode=enums.ParseMode.MARKDOWN)
                else:
                    t = "\n◍ قائمة المطورين \n≪━━━━━━━━━━━━━≫\n" + f"◍ [Black](tg://user?id={super_sudoers[0]})\n" + f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n√"
                    for row in lang:
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        Hms = await c.get_users(super_sudoers[1])
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                             Hms.first_name, user_id=super_sudoers[1]
            )
            ], 
            [ InlineKeyboardButton ( "Source Black", url=f"https://t.me/aa_ggc" )], 
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == (get_db_botname() or "فالكيري"):
        texting = ["اؤمر " + (get_db_botname() or "فالكيري") + " شتريد؟❤️🥺",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "فالكيري1") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
